...
= GCT 6.2 GridFTP: Setting up Storage Resource Broker (SRB) =
...
* **GridFTP Server** - A stock GridFTP server is loaded with the SRB-DSI. Clients contact this server to access data in a SRB resource. The server passes the request to the SRB-DSI which, as described above, passes the request on to the SRB server. The responses to the requests return along the same path.
...
* **Grid Community Toolkit** - You need the GridFTP distributed with GCT or GT (compatible with GT 4.0.1 or later and GCT 6.2 or later). You can find that link:https://github.com/gridcf/gct/releases/latest[here].
