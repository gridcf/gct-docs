...
=======================================================================
This example shows how using the Ctrl + C key combination (or other system-specific mechanism
...
be run later. 
--------
%  globus-job-run -dumprsl grid.example.org/jobmanager-pbs -np 5 -x '&(jobtype=mpi)' a.out
 &(jobtype=mpi)
    (executable="a.out")
...
[NOTE]
--
In GCT 6.2, **++globusrun++** does not print any diagnostics when given
