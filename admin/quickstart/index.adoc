...
--
.Introduction
This is a quickstart that shows a full installation of the Toolkit on
two Fedora Linux machines, named **++elephant++** and **++donkey++**. It
...
supplement the link:../../admin/install/index.html[main admin guide], 
...
users to run the Globus services.
...
The Globus Toolkit uses X.509 certificates and proxy certificates to
authenticate and authorize grid users. For this quickstart, we use the
Globus link:../../simpleca/index.html[SimpleCA] tools to manage our own
...
going to run any Globus services on **++donkey++**, then we could stop
...
First, we'll retrive a proxy credential from the myproxy server so that
